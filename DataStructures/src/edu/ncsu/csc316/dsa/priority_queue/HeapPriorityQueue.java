...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    protected void upHeap(int index) {
        ...
        while(curr > 0) {
        	int par = parent(curr);
        	if(compare(list.get(curr).getKey(), list.get(par).getKey()) >= 0) {
        		break;
        	}
        	swap(curr, par);
        	curr = par;
    ...
    protected void downHeap(int index) {
        int curr = index;
        while(hasLeft(curr)) {
        	int leftIdx = left(curr);
        	int smallIdx = leftIdx;
        	if(hasRight(curr)) {
        		int rightIdx = right(curr);
        		if(compare(list.get(leftIdx).getKey(), list.get(rightIdx).getKey()) > 0) {
        			smallIdx = rightIdx;
        		}
        	}
        	if(compare(list.get(smallIdx).getKey(), list.get(curr).getKey()) >= 0) {
        		break;
        	}
        	swap(curr, smallIdx);
        	curr = smallIdx;
